...
def apply_source_term_boundary_2d(
    ...
    b[1:, 0] = (
                        rho * 
                            (1 / dt *   (
                                            (f_e_u_left - f_wb_u) / cell_volumes[1:, 0] +
                                            (f_n_v_left - f_s_v_left) / cell_volumes[1:, 0]
                                        ) -
                                        (
                                            (f_e_u_left - f_wb_u) / cell_volumes[1:, 0])**2 -
                                    2 * (
                                            (f_n_u_left - f_s_u_left) / cell_volumes[1:, 0] *
                                            (f_e_v_left - f_wb_v) / cell_volumes[1:, 0]
                                        ) -
                                        (
                                            (f_n_v_left - f_s_v_left) / cell_volumes[1:, 0])**2
                            )
                    )

    # right wall

    f_w_u_right = face_areas_x[1:, -2] * (u[1:, -2] + u[1:, -1]) / 2
    f_eb_u = 0
    f_s_u_right = face_areas_y[1:-1, 0] * (u[:-2, -1] + u[1:-1, -1]) / 2
    f_n_u_right = face_areas_y[2:, 0] * (u[1:-1, -1] + u[2:, -1]) / 2

    f_w_v_right = face_areas_x[1:, -2] * (v[1:, -2] + v[1:, -1]) / 2
    f_eb_v = 0
    ...
    b[1:, -1] = (
                        rho * 
                            (1 / dt *   (
                                            (f_eb_u - f_w_u_right) / cell_volumes[1:, -1] +
                                            (f_n_v_right - f_s_v_right) / cell_volumes[1:, -1]
                                        ) -
                                        (
                                            (f_eb_u - f_w_u_right) / cell_volumes[1:, -1])**2 -
                                    2 * (
                                            (f_n_u_right - f_s_u_right) / cell_volumes[1:, -1] *
                                            (f_eb_v - f_w_v_right) / cell_volumes[1:, -1]
                                        ) -
                                        (
                                            (f_n_v_right - f_s_v_right) / cell_volumes[1:, -1])**2
                            )
                    )
    ...
    f_sb_u = 0
    f_n_u_bottom = face_areas_y[1, 1:] * (u[0, 1:] + u[1, 1:]) / 2

    f_w_v_bottom = face_areas_x[0, 1:-1] * (v[0, :-2] + v[0, 1:-1]) / 2
    f_e_v_bottom = face_areas_x[0, 2:] * (v[0, 1:-1] + v[0, 2:]) / 2
    f_sb_v = 0
    f_n_v_bottom = face_areas_y[1, 1:] * (v[0, 1:] + v[1, 1:]) / 2

    b[0, 1:] = (
                        rho * 
                            (1 / dt *   (
                                            (f_e_u_bottom - f_w_u_bottom) / cell_volumes[0, 1:] +
                                            (f_n_v_bottom - f_sb_v) / cell_volumes[0, 1:]
                                        ) -
                                        (
                                            (f_e_u_bottom - f_w_u_bottom) / cell_volumes[0, 1:])**2 -
                                    2 * (
                                            (f_n_u_bottom - f_sb_u) / cell_volumes[0, 1:] *
                                            (f_e_v_bottom - f_w_v_bottom) / cell_volumes[0, 1:]
                                        ) -
                                        (
                                            (f_n_v_bottom - f_sb_v) / cell_volumes[0, 1:])**2
                            )
                    )
    ...
    f_s_u_top = face_areas_y[-2, 1:] * (u[-2, 1:] + u[-1, 1:]) / 2
    f_nb_u = face_areas_y[-1, 1:] * (u[-1, 1:] + u_lid) / 2

    f_w_v_top = face_areas_x[-1, 1:-1] * (v[-1, :-2] + v[-1, 1:-1]) / 2
    f_e_v_top = face_areas_x[-1, 2:] * (v[-1, 1:-1] + v[-1, 2:]) / 2
    f_s_v_top = face_areas_y[-2, 1:] * (v[-2, 1:] + v[-1, 1:]) / 2
    f_nb_v = 0

    b[-1, 1:] = (
                        rho * 
                            (1 / dt *   (
                                            (f_e_u_top - f_w_u_top) / cell_volumes[-1, 1:] +
                                            (f_nb_v - f_s_v_top) / cell_volumes[-1, 1:]
                                        ) -
                                        (
                                            (f_e_u_top - f_w_u_top) / cell_volumes[-1, 1:])**2 -
                                    2 * (
                                            (f_nb_u - f_s_u_top) / cell_volumes[-1, 1:] *
                                            (f_e_v_top - f_w_v_top) / cell_volumes[-1, 1:]
                                        ) -
                                        (
                                            (f_nb_v - f_s_v_top) / cell_volumes[-1, 1:])**2
                            )
                    )
...
def apply_cavity_flow_boundary_2d(
    ...
    u_wb = 0
    u_e_left = (u[1:, 0] + u[1:, 1]) / 2
    v_s_left = (v[:-2, 0] + v[1:-1, 0]) / 2
    v_n_left = (v[1:-1, 0] + v[2:, 0]) / 2   

    f_wb = u_wb * face_areas_x[1:, 0]
    f_e_left = u_e_left * face_areas_x[1:, 1]
    f_s_left = v_s_left * face_areas_y[1:-1, 0]
    f_n_left = v_n_left * face_areas_y[2:, 0] 

    u_wb = 0
    u_e_left = np.where(f_e_left>0, u[1:, 0], u[1:, 1])
    u_s_left = np.where(f_s_left>0, u[:-2, 0], u[1:, 0])
    u_n_left = np.where(f_n_left>0, u[1:-1, 0], u[2:, 0])

    v_wb = 0
    v_e_left = np.where(f_e_left>0, v[1:, 0], v[1:, 1])
    v_s_left = np.where(f_s_left>0, v[:, 0], v[1:, 0])
    v_n_left = np.where(f_n_left>0, v[1:-1, 0], v[2:, 0])

    ## diffusion

    a_e_left = face_areas_x[1:, 1] / dist_x[0]
    a_wb = face_areas_x[1:, 0] / (xc[0] + lx - xc[-1])
    a_n_left = face_areas_y[2:, 0] / dist_y[1:]
    a_s_left = face_areas_y[1:-1, 0] / dist_y[:-1]

    ## pressure

    pf_wb_p = face_areas_x[1:, 0] * left[1:]
    pf_e_p_left = face_areas_x[1:, 1] * (p[1:, 0] + p[1:, 1]) / 2
    ...
    u[1:, 0] = un[1:, 0] + dt / cell_volumes[1:, 0] * (
        ...
        nu * (a_e_left * (u[1:, 1] - u[1:, 0]) - a_wb * (u[1:, 0] - u_wb) + a_n_left * (u[2:, 0] - u[1:-1, 0]) - a_s_left * (u[1:-1, 0] - u[:-2, 0]))
        ) - (dt / rho * (pf_e_p_left - pf_wb_p) / cell_volumes[1:, 0])
    
    v[1:, 0] = vn[1:, 0] + dt / cell_volumes[1:, 0] * (
        (f_e_left * v_e_left - f_wb * v_wb + f_n_left * v_n_left - f_s_left * v_s_left) + 
        nu * (a_e_left * (v[1:, 1] - v[1:, 0]) - a_wb * (v[1:, 0] - v_wb) + a_n_left * (v[2:, 0] - v[1:-1, 0]) - a_s_left * (v[1:-1, 0] - v[:-2, 0]))
        ) - (dt / rho * (pf_n_p_left - pf_s_p_left) / cell_volumes[1:, 0])

    # right wall

    ## convection

    u_w_right = (u[1:, -2] + u[1:, -1]) / 2
    u_eb = 0
    v_s_right = (v[:-2, -1] + v[1:-1, -1]) / 2
    v_n_right = (v[1:-1, -1] + v[2:, -1]) / 2   

    f_w_right = u_w_right * face_areas_x[1:, -2]
    f_eb = u_eb * face_areas_x[1:, -1]
    f_s_right = v_s_right * face_areas_y[1:-1, -1]
    f_n_right = v_n_right * face_areas_y[2:, -1] 

    u_w_right = np.where(f_w_right>0, u[1:, -2], u[1:, -1])
    u_eb = 0
    u_s_right = np.where(f_s_right>0, u[:-2, -1], u[1:-1, -1])
    u_n_right = np.where(f_n_right>0, u[1:-1, -1], u[2:, -1])

    v_w_right = np.where(f_w_right>0, v[1:, -2], v[1:, -1])
    v_eb = 0
    ...
    a_eb = face_areas_x[1:, 0] / (xc[0] + lx - xc[-1])
    a_w_right = face_areas_x[1:, -1] / dist_x[-1]
    a_n_right = face_areas_y[2:, -1] / dist_y[1:]
    a_s_right = face_areas_y[1:-1, -1] / dist_y[:-1]

    ## pressure

    pf_w_p_right = face_areas_x[1:, -2] * (p[1:, -2] + p[1:, -1]) / 2 
    pf_eb_p = face_areas_x[1:, -1] * right[1:]
    ...
    u[1:, -1] = un[1:, -1] + dt / cell_volumes[1:, -1] * (
        ...
        nu * (a_eb * (u_eb - u[1:, -1]) - a_w_right * (u[1:, -1] - u[1:, -2]) + a_n_right * (u[2:, -1] - u[1:-1, -1]) - a_s_right * (u[1:-1, -1] - u[:-2, -1]))
        ) - (dt / rho * (pf_eb_p - pf_w_p_right) / cell_volumes[1:, -1])


    v[1:, -1] = vn[1:, -1] + dt / cell_volumes[1:, -1] * (
        (f_eb * v_eb - f_w_right * v_w_right + f_n_right * v_n_right - f_s_right * v_s_right) +
        nu * (a_eb * (v_eb - v[1:, -1]) - a_w_right * (v[1:, -1] - v[1:, -2]) + a_n_right * (v[2:, -1] - v[1:-1, -1]) - a_s_right * (v[1:-1, -1] - v[:-2, -1]))
        ) - (dt / rho * (pf_n_p_right - pf_s_p_right) / cell_volumes[1:, -1])
    ...
    v_sb = 0
    v_n_bottom = (v[0, 1:] + v[1, 1:]) / 2   

    f_w_bottom = u_w_bottom * face_areas_x[0, 1:-1]
    f_e_bottom = u_e_bottom * face_areas_x[0, 2:]
    f_sb = v_sb * face_areas_y[0, 1:]
    f_n_bottom = v_n_bottom * face_areas_y[1, 1:] 
    ...
    a_e_bottom = face_areas_x[0, 2:] / dist_x[1:]
    a_w_bottom = face_areas_x[0, 1:-1] / dist_x[:-1]
    a_n_bottom = face_areas_y[1, 1:] / dist_y[0]
    a_sb = face_areas_y[0, 1:] / (yc[0] + ly - yc[-1])

    ## pressure

    pf_w_p_bottom = face_areas_x[0, 1:-1] * (p[0, :-2] + p[0, 1:-1]) / 2 
    pf_e_p_bottom = face_areas_x[0, 2:] * (p[0, 1:-1] + p[0, 2:]) / 2 
    pf_sb_p = face_areas_y[0, 1:] * bottom[1:]
    pf_n_p_bottom = face_areas_y[1, 1:] * (p[0, 1:] + p[1, 1:]) / 2

    ## update

    u[0, 1:] = dt / cell_volumes[0, 1:] * (
        (f_e_bottom * u_e_bottom - f_w_bottom * u_w_bottom + f_n_bottom * u_n_bottom - f_sb * u_sb) +
        nu * (a_e_bottom * (u[0, 2:] - u[0, 1:-1]) - a_w_bottom * (u[0, 1:-1] - u[0, :-2]) + a_n_bottom * (u[1, 1:] - u[0, 1:]) - a_sb * (u[0, 1:] - u_sb))
        ) - (dt / rho * (pf_e_p_bottom - pf_w_p_bottom) / cell_volumes[0, 1:])

    v[0, 1:] = dt / cell_volumes[0, 1:] * (
            (f_e_bottom * v_e_bottom - f_w_bottom * v_w_bottom + f_n_bottom * v_n_bottom - f_sb * v_sb) +
            nu * (a_e_bottom * (v[0, 2:] - v[0, 1:-1]) - a_w_bottom * (v[0, 1:-1] - v[0, :-2]) + a_n_bottom * (v[1, 1:] - v[0, 1:]) - a_sb * (v[0, 1:] - v_sb))
        ) - (dt / rho * (pf_n_p_bottom - pf_sb_p) / cell_volumes[0, 1:])
    ...
    a_e_top = face_areas_x[-1, 2:] / dist_x[1:]
    a_w_top = face_areas_x[-1, 1:-1] / dist_x[:-1]
    a_nb = face_areas_y[0, 1:] / (yc[0] + ly - yc[-1])
    a_s_top = face_areas_y[-1, 1:] / dist_y[-1]

    ## pressure

    pf_w_p_top = face_areas_x[-1, 1:-1] * (p[-1, :-2] + p[-1, 1:-1]) / 2 
    pf_e_p_top = face_areas_x[-1, 2:] * (p[-1, 1:-1] + p[-1, 2:]) / 2 
    pf_s_p_top = face_areas_y[-2, 1:] * (p[-2, 1:] + p[-1, 1:]) / 2
    pf_nb_p = face_areas_y[-1, 1:] * top[1:]

    ## update

    u[0, 1:-1] = un[-1, 1:] + dt / cell_volumes[0, 1:] * (
        (f_e_top * u_e_top - f_w_top * u_w_top + f_n_top * u_nb - f_s_top * u_s_top) +
        nu * (a_e_top * (u[-1, 2:] - u[-1, 1:-1]) - a_w_top * (u[-1, 1:-1] - u[-1, :-2]) + a_nb * (u_nb - u[-1, 1:]) - a_s_top * (u[-1, 1:] - u[-2, 1:]))
        ) - (dt / rho * (pf_e_p_top - pf_w_p_top) / cell_volumes[-1, 1:])

    v[0, 1:-1] = vn[-1, 1:] + dt / cell_volumes[0, 1:] * (
        (f_e_top * v_e_top - f_w_top * v_w_top + f_n_top * v_nb - f_s_top * v_s_top) +
        nu * (a_e_top * (v[-1, 2:] - v[-1, 1:-1]) - a_w_top * (v[-1, 1:-1] - v[-1, :-2]) + a_nb * (v_nb - v[-1, 1:]) - a_s_top * (v[-1, 1:] - v[-2, 1:]))
        ) - (dt / rho * (pf_nb_p - pf_s_p_top) / cell_volumes[-1, 1:])
    ...
    # u[0, 0] = dt / cell_volumes[0, 0] * (f_e_bottom_left * u_e_bottom_left - f_wb_bottom * u_wb + f_n_bottom_left * u_n_bottom_left - f_sb_left * u_sb)
    # v[0, 0] = dt / cell_volumes[0, 0] * (f_e_bottom_left * v_e_bottom_left - f_wb_bottom * v_wb + f_n_bottom_left * v_n_bottom_left - f_sb_left * v_sb)
    ...
    # u[0, -1] = dt / cell_volumes[0, -1] * (f_eb_bottom * u_eb - f_w_bottom_right * u_w_bottom_right + f_n_bottom_right * v_n_bottom_right - f_sb_right * u_sb)
    # v[0, -1] = dt / cell_volumes[0, -1] * (f_eb_bottom * v_eb - f_w_bottom_right * v_n_bottom_right + f_n_bottom_right * v_n_bottom_right - f_sb_right * v_sb)
    ...
    # u[-1, 0] = dt / cell_volumes[-1, 0] * (f_e_top_left * u_e_top_left - f_wb_top * u_wb + f_nb_left * u_nb - f_s_top_left * u_s_top_left)
    # v[-1, 0] = dt / cell_volumes[-1, 0] * (f_e_top_left * v_e_top_left - f_wb_top * v_wb + f_nb_left * v_nb - f_s_top_left * v_s_top_left)
    ...
    # u[-1, -1] = dt / cell_volumes[0, 0] * (f_eb_top * u_eb - f_w_top_right * u_w_top_right + f_nb_right * u_nb - f_s_top_right * u_s_top_right)
    # v[-1, -1] = dt / cell_volumes[0, 0] * (f_eb_top * v_eb - f_w_top_right * v_w_top_right + f_nb_right * v_nb - f_s_top_right * v_s_top_right)
